...
class KeywordExtractor(object):
    ...
    def extract_keywords(self, text):
        try:
            ...
            LOOKUP = { str(word).lower():tag.strip("'") for word,tag in pos_tags }
            #print(LOOKUP)
            # Edit end
            dc = DataCore(text=text, stopword_set=self.stopword_set, windowsSize=self.windowsSize, n=self.n, LOOKUP = LOOKUP)
            ...
            for cand in todedup:
                ...
                if len(resultSet) == self.top:
                    break
            #print([ (cand.kw,h) for (h,cand) in resultSet])
            return [ (cand.kw,h) for (h,cand) in resultSet if (len(cand.kw.split())>1 or LOOKUP.get(cand.kw.lower(),None) in {"NOUN","PNOUN"}) and satisfied(cand.kw)]
